...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--env", choices=["stable-linear", "lorenz", "pointmass-state", "pointmass"], default="stable-linear")
...
def main() -> None:
    ...
    for step in range(1, args.steps + 1):
        ...
        pred_loss, z_for_reg = compute_prediction_loss(model, obs, actions, args.protocol)
        reg_loss = regularizer_loss(
            args.regularizer,
            z_for_reg,
            projections=args.sigreg_projections,
            gamma=args.sigreg_gamma,
            max_samples=args.sigreg_max_samples,
        )
...
def compute_prediction_loss(
    ...
    if protocol == "1step":
        ...
        return pred_loss, z_obs[:, :2].reshape(-1, z_obs.shape[-1])
    ...
    if protocol == "hstep-tf":
        ...
        return pred_loss, z_obs.reshape(-1, z_obs.shape[-1])
